...
class PacmanDeepQAgent(PacmanQAgent):
    ...
    def compute_q_targets(self, minibatch, network = None, target_network=None, doubleQ=False):
        ...
        num_batch = len(minibatch)

        Q_predict = network.run(states).data
        Q_target = np.copy(Q_predict)

        exploration_bonus = 1.0 / (2.0 * np.sqrt(self.counts[states_np[:,0], states_np[:,1]] / 100.0))

        Q_next = network.run(next_states)
        Q_note_next = target_network.run(next_states)
        argmax_action = np.argmax(Q_next.data, axis=1)
        Q_argmax_action = np.choose(argmax_action, Q_note_next.data.T)

        new_Q_estimate = rewards + (1.0 - dones) * self.discount * Q_argmax_action + exploration_bonus
        Q_target[np.arange(num_batch), actions] = new_Q_estimate
